...
export function pfad(p: Punkt[]): string {
  const n = p.length
...
/** Ruheform einer Ziffer. Unbekannte Zeichen fallen auf die 0 zurück,
    damit der Timer nie ohne Pfad dasteht. */
export function formFuer(zeichen: string): Punkt[] {
  return ZIFFERN[zeichen] ?? ZIFFERN['0']
}

/** Wählt für einen Übergang unter allen Varianten, die dasselbe zeichnen,
    die mit dem kürzesten Gesamtweg: die Laufrichtung des Ziels und — wenn
    der Ausgangsstand eine geschlossene Form ist — dessen Startpunkt auf
    dem Ring. Ohne diese Wahl wandern die Punkte quer übereinander und die
    Ziffer klappt unterwegs sichtbar zusammen (etwa 0 → 9 zu einem Winkel). */
export function besteVariante(stand: Punkt[], zeichen: string): { start: Punkt[]; ziel: Punkt[] } {
  const vorwaerts = formFuer(zeichen)
...
/** Zwischenform: jeder Punkt anteilig auf dem Weg zu seinem Gegenstück. */
export function mische(von: Punkt[], nach: Punkt[], t: number): Punkt[] {
  return von.map((p, i) => [p[0] + (nach[i][0] - p[0]) * t, p[1] + (nach[i][1] - p[1]) * t] as Punkt)
}
